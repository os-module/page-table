use crate::area::Area;
use crate::entry::{PTEFlags, PTEFlagsBuilder, PTELike, PageTableEntry};
use crate::error::PTableError;
use crate::table::PageTable;
use crate::Result;
use crate::{vpn_f_c_range, PageManager, VPNToSlice, PPN, VPN};
...
impl AddressSpace {
    ...
    pub fn recycle(&mut self) {
        self.page_manager
            .ppn_set
            .iter()
            .for_each(|x| self.page_manager.dealloc(*x))
    }
    ...
    pub fn copy_from_other(address_space: &AddressSpace) -> Result<Self> {
        ...
        for (vpn, perm) in address_space.single_vpn.iter() {
            ...
            unsafe { new_physical.copy_from(old_physical, 4096) }
        }
    ...
    fn map(&mut self, map_area: &Area, flag: bool, is_valid: bool) -> Vec<PPN> {
        ...
        for (vpn, ppn) in map_area.iter() {
            ...
            page_table[slice[slice.len() - 1]] =
                PageTableEntry::new(ppn, valid | PTEFlags::from_bits(map_permission).unwrap());
            if flag {
                map.push(ppn);
            }
        }
    ...
    pub fn find_pte(&self, vpn: VPN) -> Option<PageTableEntry> {
        let slice = vpn.to_slice();
        let mut page_table = PageTable::from_ppn(self.root_ppn.unwrap());
        for i in 0..slice.len() - 1 {
            let pte = page_table[slice[i]];
    ...
    pub fn virtual_to_physical(&self, virtual_address: usize) -> Option<usize> {
        ...
        self.vpn_to_ppn(vpn)
            .map(|ppn| ppn.to_address() + (virtual_address & 0xfff))
    }
    ...
    pub fn unmap(&mut self, map_area: &Area) -> Result<()> {
        ...
        for (vpn, _) in map_area.iter() {
            ...
            page_table[slice[slice.len() - 1]] =
                PageTableEntry::new(PPN::new(0), PTEFlags::from_bits(0).unwrap());
        }
        // delete map_area
                        ...
                        if pte.is_valid() {
                            ...
                            for k in 0..512 {
                                ...
                                if pte.is_valid() {
                                    ...
                                    write!(
                                        f,
                                        "{1:0$}{2:03x}:{3:012x} {4:<8?}\n",
                                        space,
                                        "",
                                        k,
                                        pte.ppn().0,
                                        PTEFlagsBuilder(pte.flag())
                                    )?;
                                }
                            }
                            write!(f, "{:->67}\n", "-")?;
